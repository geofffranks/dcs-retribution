...
class TransferModel(QAbstractListModel):
    ...
    def _authorized(player: Player, settings: Any) -> bool:
        """Whether the UI may mutate transfers for the given owner right now.

        BLUE transfers are always manageable. RED transfers are manageable only
        while ``enable_enemy_buy_sell`` is true. Neutral transfers are denied.
        """
        if player.is_blue:
            return True
        if player.is_red:
    ...
    def can_manage(self, transfer: TransferOrder) -> bool:
        """Whether the UI may mutate the given transfer right now.

        Dialog button and context-menu enablement delegate here so that the view
        and the mutation gate agree on authorization.
        """
        if self.game_model.game is None:
            return False
        return self._authorized(transfer.player, self.game_model.game.settings)

    def new_transfer(self, transfer: TransferOrder, now: datetime) -> None:
        """Updates the game with the new unit transfer."""
        # Authorization is checked before Qt begin-row signals, collection
        # changes, base inventory changes, or refunds.
        self._assert_authorized(transfer)
        # BLUE rows always precede RED rows: a BLUE insertion announces the
        # pre-insert BLUE count, a RED insertion announces the aggregate tail.
        ...
        # The model is the sole publisher: one accumulator drives the nested
        # mutation (which may plan an airlift) and is published exactly once.
    ...
    def cancel_transfer(self, transfer: TransferOrder) -> None:
        ...
        self.beginRemoveRows(QModelIndex(), index, index)
        # The model is the sole publisher: one accumulator drives the nested
        # mutation (which may cancel an airlift) and is published exactly once.
        events = GameUpdateEvents()
        # TODO: Needs to regenerate base inventory tab.
