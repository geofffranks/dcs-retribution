...
def test_red_transfer_destination_combo_uses_red_friendly_points(
    ...
    game = cast(
        ...
        SimpleNamespace(
            ...
            # The unified dialog keeps the inventory-status destination filter,
            # which also requires a transit path; the stub network always has
            # one so only the owner checks are exercised here.
            transit_network_for=lambda _owner: SimpleNamespace(
...
def test_red_transfer_model_rejects_mutation_when_cheat_is_disabled() -> None:
    ...
    game = SimpleNamespace(
        settings=SimpleNamespace(enable_enemy_buy_sell=False),
        coalition_for=lambda _player: SimpleNamespace(transfers=transfers),
    )
    model = TransferModel(cast(Any, SimpleNamespace(game=game)))
    transfer = cast(Any, SimpleNamespace(player=Player.RED))

    # The unified TransferModel keeps the inventory-status behavior its final
    # review approved: an unauthorized RED mutation is a silent no-op rather
    # than a PermissionError, so the collection is never consulted.
    model.new_transfer(transfer, SimpleNamespace())

    transfers.new_transfer.assert_not_called()
