...
def test_motorpool_targets_projects_dead_groups_with_live_reserve() -> None:
    gut = _gut()
    target, enemy_cp = _motorpool_cp({gut: 4}, friendly=False)
    target.groups = [MagicMock(alive_units=0)]
    game = _game([enemy_cp, _friendly_cp()])

    # Groups are ephemeral render artifacts; the reserve pool is authoritative,
    # so dead-only groups with a live reserve still render next mission and the
    # motorpool stays targetable.
    assert list(
        ObjectiveFinder(cast("Game", game), Player.BLUE).motorpool_targets()
    ) == [target]
...
def test_motorpool_attack_sizes_from_shared_cp_allocation() -> None:
    ...
    recon = next(f for f in task.flights if f.task is FlightType.ARMED_RECON)
    assert recon.num_aircraft == 2  # five of the shared ten-unit cap go to this TGO
