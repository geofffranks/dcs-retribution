...
def _game_model(game: Any) -> Any:
    return MagicMock(game=game)
...
def test_denied_mutation_emits_no_model_signal(app: QApplication) -> None:
    """Mutation denial leaves the dialog open, emits no row signal, changes nothing."""
    blue = _make_pending(Player.BLUE)
    red = _make_pending(Player.RED)
    red_origin = _cp("Red Origin", captured=Player.RED)
    red_dest = _cp("Red Dest", captured=Player.RED)
    ...
    with pytest.raises(PermissionError):
        model.cancel_transfer(red.pending_transfers[0])

    assert inserts == []
    assert removals == []
    assert len(red.pending_transfers) == 1
