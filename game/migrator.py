...
class Migrator:
    ...
    def _normalize_transfers_for_coalition(
        ...
        # Traverse pending list plus convoy/cargo aliases, deduplicating by
        # object identity. The same transfer object is normally reachable from
        # both the pending list and a convoy/cargo alias.
        ...
        # Reject the same transfer reachable from more than one coalition.
        for transfer in all_transfers:
            if id(transfer) in global_seen:
                raise RuntimeError(
                    f"Transfer {transfer} is reachable from more than one " f"coalition"
                )
    ...
    def _normalize_single_transfer_player(transfer: Any, coalition: Any) -> None:
        ...
        if owner is None or isinstance(owner, bool):
            # Missing and legacy-boolean owners are set from the containing
            # coalition, not the origin's current owner.
