        ...
        for cp in control_points:
            for tgo in cp.ground_objects:
                if not isinstance(tgo, MotorpoolGroundObject):
                    continue
                # Built from a PresetLocation, so the position is heading-aware
                # at runtime even though MissionTarget types it as a plain Point.
                identity = motorpool_identity(
                    tgo.original_name,
                    typing.cast("PointWithHeading", tgo.position),
                )
