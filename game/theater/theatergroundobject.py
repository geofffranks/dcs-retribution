...
def motorpool_rendered_unit_count(
    ...
    """Return the units in the renderer's next motorpool snapshot.

    Always projects the live reserve pool: the reserve is the current source of
    truth, while ``tgo.groups`` are ephemeral artifacts of the previous
    mission's render that may be stale in either direction (losses already
    decremented the reserve; replenishment raised it).
    """
